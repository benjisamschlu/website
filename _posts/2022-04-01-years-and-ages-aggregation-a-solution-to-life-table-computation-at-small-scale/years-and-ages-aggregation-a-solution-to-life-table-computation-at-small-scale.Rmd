...
description: |
  Runing a Monte-Carlo simulation to quantify the uncertainty reduction in life expectancy when aggregating over ages and years.
author:
...
When aggregating over years to compute life tables, we assume a constant mortality, a constant population size and a stable population structure.
...
The figure below shows 95\% and 80\% confidence intervals for life expectancy at birth associated to the six population sizes considered. Top left panel consists of building life tables on 1 year $\times$ 1year age group, top right panel on 4 years $\times$ 1 year age group, bottom left on 1 year $\times$ 5 years age group and bottom right on 4 years $\times$ 5 years age group. Aggregating over 4 years (top right panel) is clearly not sufficient to reduce uncertainty at the municipality level but also at the NUTS3 level for the smallest population sizes in this group. Focusing now on the bottom right panel, despite a significant reduction in uncertainty, none of the confidence interval's width for municipalities are below 1 year. Hence, substantial stochasticity remains for such small areas.
...
Aggregating over ages and years sometimes lead to present life table functions of small areas as deterministic. Using a Monte-Carlo simulation, we showed that performing such aggregation indeed efficiently reduces uncertainty. However, at the smallest administrative level considered (municipality), these measures are still subject to substantial stochasticity and uncertainty intervals should be presented. We did not consider aggregating over more years and ages. In the former case, we thought that in a context of diminishing mortality, aggregating over more than 4 years won't be so informative on the true mortality level experienced at a given time point. In the latter case, grouping ages together requires to assume that mortality rates are constant within age groups, which is an unrealistic assumption when more than five years are considered. Please note that these two later points are personnal opinions.
