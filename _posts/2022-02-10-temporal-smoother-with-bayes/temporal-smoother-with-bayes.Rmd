...
description: |
  Showing that Bayesian modeling makes it straightforward to implement smoothing.
author:
...
Suppose that we are interested in the population's proportion having a specific characteric at a given time point (called the prevalence). Further assume that it would be too costly or too time consuming to survey all individuals in the population of interest. Hence, only a sample of the population is surveyed to assess the prevalence over time. 

The figure below shows points reflecting the sampled prevalence over time with the true population's prevalence characterized by an exponential decay. 
...
Values of $\phi$ close to $\frac{1}{2}(\phi_{t-1} + \phi_{t+1})$ are favored. From the above expression, it is clear that $\sigma^2$ can be seen as a smoothing parameter. Small (large) value enforces strong (weak) smoothing. 
