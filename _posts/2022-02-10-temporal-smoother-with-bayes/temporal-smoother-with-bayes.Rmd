...
Suppose that we are interested in the population's proportion having a specific characteric at a given time point (called the prevalence). Further assume that it would be too costly or too time consuming to survey all individuals in the population of interest at each time point. Hence, only a sample of the population is surveyed over 81 time points to assess the prevalence over time. 

The figure below shows points reflecting the sampled prevalence over time. On the left, the sample size is fixed at $n=100$ while on the right it is $n=1000$. The dashed line represents the true population's prevalence characterized by an exponential decay as time progress. 
...
where we have from left to right, the posterior distribution, the likelihood and the prior distribution. The likelihood describes the distribution of the data, depending on unknown parameters $\theta$. The prior distribution expresses beliefs about $\theta$ and, of main interest for this post, provides a mechanism by which smoothing can be imposed.
...
where $y_t$ is the number of individuals having the characteristic at time $t$ out of $n=100$ sampled individuals (note that $n$ is fixed over time), $p_t$ is the prevalence we want to estimate, $a$ consists of an intercept and $\phi_t$ is a parameter that changes over time. **Here comes the prior distribution as a mechanism to impose smoothing:** we will assume that $\phi_t$ is distributed as a random walk of order one. This assumption encourages $\phi_t$ at $t$ to be similar to its neighbors. The prior can thus be expressed as
...
According to the selected prior distribution, values of $\phi_t$ close to $\frac{1}{2}(\phi_{t-1} + \phi_{t+1})$ are favored. It is clear from that distribution that $\sigma^2$ can be seen as a smoothing parameter since it defines the spread around $\frac{1}{2}(\phi_{t-1} + \phi_{t+1})$. Small (large) value of $\sigma$ enforces strong (weak) smoothing. 
...
model {
  ...
  phi[1] ~ normal(0, 10); // Random walk 1 for phi
...
In the figure below we show the estimated prevalence (with 95\% credible interval) where we imposed different priors on $\sigma$ . On the left, we assumed that $\sigma \sim \mathcal{N}^+(0,10)$ while one the right, we imposed more smoothing by setting $\sigma \sim \mathcal{N}^+(0,0.01)$.
...
df.fit.unp <- fit %>% 
  ...
  median_qi() %>% 
  mutate(type = "sigma~N(0,10)")
df.fit.pen <- fit.pen %>% 
  spread_draws(p_hat[t]) %>% 
  median_qi() %>% 
  mutate(type = "sigma~N(0,0.01)")

df.fit <- bind_rows(df.fit.unp, 
                    df.fit.pen) %>% 
  mutate(type = factor(type,
                       levels = c("sigma~N(0,10)", "sigma~N(0,0.01)"))) %>% 
  left_join(df.stan %>% 
              select(t, p, p.sim),
            by = "t")
...
The figure clearly shows that prior can be used to increase smoothing of our estimated prevalence. Indeed, the right side of the plot show much less wiggle than on the left when $\sigma \sim \mathcal{N}(0,10)$.
